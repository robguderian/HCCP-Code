...
Tx64Request::Tx64Request(XBeeAddress64 &addr64, uint8_t *data, uint8_t dataLength) : PayloadRequest(TX_64_REQUEST, DEFAULT_FRAME_ID, data, dataLength) {
	_addr64 = addr64;
	//_option = ACK_OPTION; // <-- rg - testing no acks
...
void XBee::send(XBeeRequest &request) {
	...
	// send length
	uint8_t msbLen = ((request.getFrameDataLength() + 2) >> 8) & 0xff; // < ---- modified by rg to make it 802.15.4 compliant. 
	uint8_t lsbLen = (request.getFrameDataLength() + 2) & 0xff;

	sendByte(msbLen, true); // <-- leave this... needs to turn into a frame control header.
	...
	sendByte(request.getFrameId(), true);
        ...
        //sendByte(0,true); // rg for sunspots.
